...
class AvisModel
{
    ...
    {
        // Quand on refuse un avis, on le retire de l'accueil en même temps
        $retirer = $status !== 'valide' ? 1 : 0;
        $stmt = Database::getConnection()->prepare(
            "UPDATE avis SET statut = ?, afficher_accueil = IF(? = 1, 0, afficher_accueil) WHERE commande_id = ?"
        );
        $stmt->execute([$status, $retirer, $commandeId]);
    }
