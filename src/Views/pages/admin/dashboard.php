...
<div class="container py-4">
    ...
    <div class="card">
        ...
        <div class="card-body p-0">
            <?php if (empty($activiteRecente)): ?><p class="text-muted p-3 mb-0">Aucune activité.</p><?php else: ?><ul class="list-group list-group-flush"><?php foreach ($activiteRecente as $cmd): ?><li class="list-group-item py-3 px-3"><div class="d-flex justify-content-between align-items-center gap-3"><div class="min-w-0"><div class="fw-medium text-truncate"><?= sanitize($cmd['numero_commande'] ?? '') ?></div><div class="text-muted small text-truncate"><?= sanitize(personFullName($cmd)) ?> · <?= sanitize($cmd['menu_titre'] ?? '') ?> · <?= sanitize(formatDateFr($cmd['date_prestation'] ?? null)) ?></div></div><div class="d-flex align-items-center gap-2"><?= commandeStatusBadge($cmd['statut'] ?? null) ?><span class="fw-semibold text-brand text-nowrap"><?= sanitize(formatMoneyCents($cmd['prix_total_cents'] ?? 0)) ?></span></div></div></li><?php endforeach; ?></ul><?php endif; ?>
        </div>
    </div>
</div>
