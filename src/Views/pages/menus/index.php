...
<div class="container py-5 menus-page">
    ...
    <aside class="filter-bar mb-5" aria-label="Filtres de recherche">
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
            <h2 class="h6 mb-0 fw-semibold"><i class="bi bi-funnel me-2"></i>Trouver le bon menu</h2>
            <button id="reset-filtres" class="btn btn-sm btn-outline-secondary">Réinitialiser</button>
        </div>
        <div class="row g-3">
            <div class="col-12 col-md-6 col-xl">
                ...
                <select id="filtre-theme" class="form-select filtre">
                    ...
                    <?php foreach ($themes as $t): ?>
                        <option value="<?= $t['theme_id'] ?>" <?= ($filters['theme_id'] ?? '') == $t['theme_id'] ? 'selected' : '' ?>><?= sanitize($t['libelle']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="col-12 col-md-6 col-xl">
                ...
                <select id="filtre-regime" class="form-select filtre">
                    ...
                    <?php foreach ($regimes as $r): ?>
                        <option value="<?= $r['regime_id'] ?>" <?= ($filters['regime_id'] ?? '') == $r['regime_id'] ? 'selected' : '' ?>><?= sanitize($r['libelle']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="col-12 col-md-6 col-xl">
    ...
    <div id="menus-container" class="row g-4" role="list" aria-live="polite" aria-label="Liste des menus">
        <?php foreach ($menus as $menu): ?>
            <div class="col-12 col-md-6 col-lg-4" role="listitem">
                <article class="card menu-card h-100">
                    <img src="<?= sanitize(imageUrl($menu['image_principale'] ?: null)) ?>" class="card-img-top" alt="Illustration du menu <?= sanitize($menu['titre']) ?>" loading="lazy" decoding="async">
                    <div class="card-body d-flex flex-column">
                        <h3 class="card-title h5"><?= sanitize($menu['titre']) ?></h3>
                        <p class="card-text text-muted small flex-grow-1"><?= sanitize(substr(html_entity_decode($menu['description'] ?? '', ENT_QUOTES, 'UTF-8'), 0, 120)) ?>...</p>
                        <div class="mt-3">
                            <?php
                            ...
                            ?>
                            <div><span class="menu-price"><?= sanitize(formatPrice($menu['prix_par_personne'] ?? 0)) ?></span><small class="text-muted"> / personne</small></div>
                            ...
                            <?php if ($menu['quantite_restante'] !== null): ?>
                                ...
                                <?php endif; ?>
                            <?php endif; ?>
                            <a href="/menus/detail?id=<?= (int) $menu['menu_id'] ?>" class="btn btn-brand btn-sm mt-3">Voir le détail</a>
                        </div>
                    </div>
                </article>
            </div>
        <?php endforeach; ?>
        <?php if (empty($menus)): ?>
            <div class="col-12 text-center text-muted py-5"><i class="bi bi-search display-4 mb-3 d-block"></i><p>Aucun menu ne correspond à vos critères.</p></div>
        <?php endif; ?>
    </div>
</div>
...
filtres.forEach(filter => filter.addEventListener('input', () => {
    clearTimeout(debounceTimer);
    debounceTimer = setTimeout(fetchMenus, 400);
}));
filtres.forEach(filter => filter.addEventListener('change', fetchMenus));

function fetchMenus() {
    if (menusRequestController) menusRequestController.abort();
    ...
    container.innerHTML = '<div class="col-12 text-center py-5"><div class="spinner-border text-brand" role="status"><span class="visually-hidden">Chargement...</span></div></div>';

    window.tugeresFetchJson('/menus?' + params.toString(), {signal: requestController.signal})
        .then(payload => {
            ...
            container.innerHTML = menus.map(menu => `
                <div class="col-12 col-md-6 col-lg-4" role="listitem">
                    <article class="card menu-card h-100">
                        <img src="${(menu.image_principale && (menu.image_principale.startsWith('http://') || menu.image_principale.startsWith('https://'))) ? esc(menu.image_principale) : '/' + esc(menu.image_principale || 'images/menu-placeholder.webp')}" class="card-img-top" alt="Illustration du menu ${esc(menu.titre)}" loading="lazy" decoding="async">
                        <div class="card-body d-flex flex-column">
                            <h3 class="card-title h5">${esc(menu.titre)}</h3>
                            <p class="card-text text-muted small flex-grow-1">${menu.description ? esc(menu.description.substring(0, 120)) + '...' : ''}</p>
                            <div class="mt-3">
                                <div><span class="menu-price">${Number(menu.prix_par_personne).toLocaleString('fr-FR', {minimumFractionDigits: 2, maximumFractionDigits: 2})} €</span><small class="text-muted"> / personne</small></div>
                                <p class="small text-muted mt-2 mb-0">${requestedGuests > 0 ? `Estimation pour ${parseInt(menu.personnes_estimees)} personnes : <strong>${Number(menu.prix_estime).toLocaleString('fr-FR', {minimumFractionDigits: 2, maximumFractionDigits: 2})} €</strong>` : `À partir de ${parseInt(menu.nombre_personne_minimum)} personnes : <strong>${Number(menu.prix_estime).toLocaleString('fr-FR', {minimumFractionDigits: 2, maximumFractionDigits: 2})} €</strong>`}</p>
                                ${menu.quantite_restante !== null ? (parseInt(menu.quantite_restante) <= 0 ? `<p class="small text-danger fw-semibold mt-2 mb-0"><i class="bi bi-x-circle me-1"></i>Plus disponible</p>` : parseInt(menu.quantite_restante) <= 3 ? `<p class="small text-warning fw-semibold mt-2 mb-0"><i class="bi bi-exclamation-circle me-1"></i>Plus que ${parseInt(menu.quantite_restante)} place(s)</p>` : `<p class="small text-success mt-2 mb-0"><i class="bi bi-check-circle me-1"></i>${parseInt(menu.quantite_restante)} places disponibles</p>`) : ''}
                                <a href="/menus/detail?id=${parseInt(menu.menu_id)}" class="btn btn-brand btn-sm mt-3">Voir le détail</a>
                            </div>
                        </div>
                    </article>
                </div>
