...
<?php $workspaceActive = isAuth() && roleWorkspaceIsActive(); ?>
<nav class="navbar navbar-expand-xl navbar-dark bg-brand sticky-top site-navbar" aria-label="Navigation principale">
    <div class="container">
            ...
            <ul class="navbar-nav main-nav-list me-auto mb-0">
                ...
                <li class="nav-item">
                    <a class="nav-link <?= routeIsActive('/') ? 'active' : '' ?>" href="/" <?= routeIsActive('/') ? 'aria-current="page"' : '' ?>>
                        <i class="bi bi-house-door nav-link-icon d-xl-none" aria-hidden="true"></i><span>Accueil</span>
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link <?= routeIsActive('/menus*') ? 'active' : '' ?>" href="/menus" <?= routeIsActive('/menus*') ? 'aria-current="page"' : '' ?>>
                        <i class="bi bi-journal-richtext nav-link-icon d-xl-none" aria-hidden="true"></i><span>Tous les menus</span>
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link <?= routeIsActive('/contact') ? 'active' : '' ?>" href="/contact" <?= routeIsActive('/contact') ? 'aria-current="page"' : '' ?>>
                        <i class="bi bi-chat-left-text nav-link-icon d-xl-none" aria-hidden="true"></i><span>Contact</span>
                    </a>
                </li>
            </ul>
                ...
                <?php if (isAuth()): ?>
                    ...
                    <li class="nav-item me-1">
                        ...
                        <a class="nav-link nav-link-cart" href="/panier" aria-label="Votre panier (<?= $panierCount ?> article<?= $panierCount > 1 ? 's' : '' ?>)">
                            <span class="cart-icon-wrap">
                                ...
                                <?php if ($panierCount > 0): ?><span class="cart-badge"><?= $panierCount ?></span><?php endif; ?>
                            </span>
                    ...
                    <li class="nav-item">
                        ...
                        <a class="nav-link <?= $navActive ? 'active' : '' ?>" href="<?= sanitize($navHref) ?>" <?= $navActive ? 'aria-current="page"' : '' ?>>
                            <i class="bi bi-person-workspace nav-link-icon d-xl-none" aria-hidden="true"></i><span><?= sanitize(roleHomeLabel()) ?></span>
                        </a>
                    </li>
                    <li class="nav-item">
                        <form method="POST" action="/deconnexion" class="m-0">
                            ...
                            <button type="submit" class="nav-link nav-link-logout border-0 bg-transparent">
                                <i class="bi bi-box-arrow-right nav-link-icon d-xl-none" aria-hidden="true"></i><span>Déconnexion</span>
                            </button>
                        </form>
                    </li>
                <?php else: ?>
                    ...
                    <li class="nav-item">
                        <a class="nav-link <?= routeIsActive('/connexion') ? 'active' : '' ?>" href="/connexion" <?= routeIsActive('/connexion') ? 'aria-current="page"' : '' ?>>
                            <i class="bi bi-person-circle nav-link-icon d-xl-none" aria-hidden="true"></i><span>Connexion</span>
                        </a>
                    </li>
                <?php endif; ?>
            </ul>
...
<footer class="site-footer text-light py-4 mt-5" role="contentinfo">
    <div class="container">
        <div class="footer-grid">
            <?php if (!empty($siteHoraires)): ?>
                <section class="footer-section" aria-labelledby="footer-horaires">
                    ...
                    <ul class="footer-hours list-unstyled mb-0">
                        <?php foreach ($siteHoraires as $h): ?>
                            <li><strong><?= sanitize($h['jour']) ?> :</strong> <?= $h['heure_ouverture'] === 'Fermé' ? '<span class="text-danger">Fermé</span>' : sanitize($h['heure_ouverture']) . ' - ' . sanitize($h['heure_fermeture']) ?></li>
                        <?php endforeach; ?>
                    </ul>
                </section>
            <?php endif; ?>
